...
static int node_reflectedVelocityX(double velocityX, int inputTick, int inputLBtn, int inputRBtn, double positionX_at_last, double* output) {
  int _tmp017;
  double _tmp016;
  _tmp017 = _vertial__vertial_(inputTick, _vertial__vertial_(inputLBtn, inputRBtn));
  if (_tmp017 == 1) {
    int _tmp019;
    double _tmp018;
    _tmp019 = _vertial__vertial_(_anpersand_anpersand(_lt__dot_(_minus__dot_(positionX_at_last, ballRadius), leftWallPos), _lt__dot_(velocityX, 0.0)), _anpersand_anpersand(_gt__dot_(_plus__dot_(positionX_at_last, ballRadius), rightWallPos), _gt__dot_(velocityX, 0.0)));
    if (_tmp019 == 1) {
      _tmp018 = _at__minus__dot_(e);
    }
    else {
      _tmp018 = 1.0;
    }
  ...
  *output = _asterisk__dot_(velocityX, _tmp016);
  return 1;
}
static double init_reflectedVelocityX() {
  return 0.0;
...
static int node_positionX(double positionX_at_last, int inputTick, double reflectedVelocityX, double dt, double* output) {
  double _tmp020;
  if (inputTick == 1) {
    _tmp020 = _asterisk__dot_(reflectedVelocityX, dt);
  }
  else {
    _tmp020 = 0.0;
  }
  *output = _plus__dot_(positionX_at_last, _tmp020);
  return 1;
}
static double init_positionX() {
  return 0.0;
}
static int node_positionY(double positionY_at_last, int inputTick, double reflectedVelocityX, double dt, double* output) {
  double _tmp021;
  if (inputTick == 1) {
    _tmp021 = _asterisk__dot_(reflectedVelocityX, dt);
  }
  else {
    _tmp021 = 0.0;
  }
  *output = _plus__dot_(positionY_at_last, _tmp021);
  return 1;
}
static double init_positionY() {
  return 80.0;
...
void ActivateBounceSimulator() {
  ...
  while (1) {
    ...
    Counter++;
    node_positionY(node_memory_positionY[last_side], node_memory_inputTick[current_side], node_memory_reflectedVelocityX[current_side], node_memory_dt[current_side], &node_memory_positionY[current_side]);
    Counter++;
